...
const PageList_Receipt = () => {
    ...
    // Load receipt data using POST API
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const apiUrl = `${API_WEB_URLS.BASE}GetReceiptData/${loggedUserId}/token`;
    ...
    const handleAdd = () => {
        navigate("/receipt", { state: { Id: 0 } });
    };

    const handleEdit = (id: number | string) => {
        if (!id) return;
        navigate("/receipt", { state: { Id: id } });
    };
    ...
    return (
        <div className="page-body">
            <Breadcrumbs mainTitle="Receipt Vouchers" parent="Transactions" />
            <Container fluid>
                <Row>
                    <Col xs="12">
                        <Card>
                            <CardHeaderCommon title="Receipt Voucher List" tagClass="card-title mb-0" />
                            <CardBody>
                                {/* Filter Row */}
                                <Row className="mb-3">
                                    <Col md="3">
...
export default PageList_Receipt;
