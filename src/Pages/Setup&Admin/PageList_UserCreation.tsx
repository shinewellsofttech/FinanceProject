...
const LIST_API_URL = `${API_WEB_URLS.MASTER}/0/token/UserMaster/Id/0`;
const DELETE_API_URL = `${API_WEB_URLS.MASTER}/0/token/UserMaster/Id`;
...
const PageList_UserCreation = () => {
  ...
  const handleAdd = () => {
    navigate("/userCreation", { state: { Id: 0 } });
  };

  const handleEdit = (id: number | string) => {
    if (!id) return;
    navigate("/userCreation", { state: { Id: id } });
  };
  ...
  return (
    <div className="page-body">
      <Breadcrumbs mainTitle="User Creation" parent="Setup & Admin" />
      <Container fluid>
        <Row>
          <Col xs="12">
            <Card>
              <CardHeaderCommon title="User List" tagClass="card-title mb-0" />
              <CardBody>
                <Row className="mb-3">
...
export default PageList_UserCreation;
